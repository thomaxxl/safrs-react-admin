...
const KCDefaultApp = ({keycloakClient}:{keycloakClient:any}) => {
  ...
  const dataProvider = jsonapiClient(conf.api_root, { conf: {} }, keycloakClient);
  ...
  console.log('kcstarting', loading, conf.authentication?.keycloak, keycloakClient?.token)
...
const App: React.FC = () => {
  ...
  const initKeyCloakClient = async (conf:any) => {
    ...
    console.log('initKeycloakClient', keycloakClient, conf)
    
    if(keycloakClient.authenticated && keycloakClient.token){
        console.log('KcAuthenticated', keycloakClient.token)
        return
    }
  ...
  if(keycloak === undefined){
    return <Loading loadingPrimary="Authenticating..." loadingSecondary="Please wait" />;
  }
  
  
  return <KCDefaultApp keycloakClient={keycloak} />
}
