 ...
 */
async function fetchCode(url: any) {
 ...
 */
async function createModuleFromURL(url: any, context: any) {
 ...
 */
async function linkWithImportMap({ imports }: { imports: any }) {
  /**
   * @param {string} specifier
   * @param {vm.SourceTextModule} referencingModule
   * @returns {Promise<vm.SourceTextModule>}
   */
  return async function link(specifier: any, referencingModule: any) {
    let url: any;
...
export default async function dynamicImport(
  specifier: string,
  sandbox: object = {},
  { imports = {} }: { imports: { [key: string]: string } } = { imports: {} }
) {
