...
const prepareAttributes = (attributes: any, resource_en: any) => {
  ...
  const resource_attr_rels = conf.resources[resource].attributes?.map(
    (attr: any) => (attr.relationship ? attr.name : null)
  );
  ...
  for (let [k, v] of Object.entries(attributes)) {
    ...
    if (
      typeof v === "number" &&
      resource_attr_rels.find((n: string) => n == k)
    ) {
...
export const jsonapiClient = (
  ...
  return {
    ...
    getList: (resource, params) => {
      try {
        ...
        if (params.filter?.q && "resources" in conf) {
          ...
          const filter =
            search_cols?.map((col: any) => {
              return {
                name: col.name,
                op: col.op ? col.op : "ilike",
                val: col.val
                  ? col.val.format(params.filter.q)
                  : `%${params.filter.q}%`,
              };
            }) || "";
          if (filter) {
        ...
        const includes: string[] = rel_conf
          .filter((rel: any) => rel.direction != "tomany")
    ...
    ********************************************************************************************/
    getMany: (resource_en, params: any) => {
      try {
        if (
          resource_en === null ||
          resource_en === undefined ||
          resource_en === "" ||
          resource_en === "Location"
        ) {
          return new Promise(() => {
            return { data: {} };
          });
        ...
        return httpClient(url, {}).then(({ json }) => {
          ...
          const jsonData = json.data.map((value: any) =>
            Object.assign(
              ...
              prepareAttributes(value.attributes, resource)
            )
          );
          const validUntil = new Date();
          validUntil.setTime(validUntil.getTime() + duration);
    ...
    ********************************************************************************************/
    getManyReference: (resource_name_en, params: any) => {
      try {
        ...
        const includes: string[] = rel_conf
          .map((rel: any) => rel.name)
          .join(",");
        const url = `${apiUrl}/${resource_name}?${stringify(
          query
        )}&include=${includes}`;
    ...
    //create a new record for create api
    create: (resource_name_en: string, params: any) => {
      try {
        console.log(resource_name_en, params, "create");
        const resource_name = decodeURI(resource_name_en);
        let type = conf["resources"]?.[resource_name]?.type ?? resource_name;
