...
export const setCurrentConf = (conf: Config): boolean => {
  ...
  if(isNode){
    //loadYaml('http://localhost:8282/ui/admin/admin.yaml?v=1');
    return true
  }
...
export const loadHomeConf = async (confPath = "") => {
  ...
  if(isNode){
    confPath = confPath ?? `http://localhost:8282/${projectId}/ui/admin/admin.yaml?v=2`;
  } 
  else if(confPath === ""){
    confPath = window.location.origin + `/${projectId}/ui/admin/admin.yaml?v=${new Date().getTime()}`;
    if (window.location.origin === "http://localhost:3000") {
        console.log("DEVMODE!!!");
        confPath = `http://localhost:8282/${projectId}/ui/admin/admin.yaml?v=3`;
    }
  }
